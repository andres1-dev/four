  ...
  `;
    ...
    `;
        ...
        item.datosSiesa.forEach((siesa, index) => {
            ...
            } else if (siesa.confirmacion && String(siesa.confirmacion).trim().toUpperCase() === "ENTREGADO") {
                estadoConf = "ENTREGADO";
                statusClass = "status-entregado";
            } else if (tieneIh3) {
                // Si tiene imagen pero no confirmacion, marcar como entregado
                estadoConf = "ENTREGADO";
                statusClass = "status-entregado";
            } else if (!tieneFactura) {
                estadoConf = "NO FACTURADO";
                statusClass = "status-nofacturado";
            } else if (siesa.confirmacion && String(siesa.confirmacion).includes("PENDIENTE FACTURA")) {
