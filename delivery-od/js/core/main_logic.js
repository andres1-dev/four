...
async function obtenerDatosFacturados(fechaInicio = null) {
    try {
        const [datosData2, datosSiesa, datosSoportes, datosRec] = await Promise.all([
            obtenerDatosDeData2(fechaInicio),
            obtenerDatosSiesaSupabase(),
            obtenerDatosSoportes(),
            obtenerDatosRecFiltrados(fechaInicio)
        ]);
...
async function fetchIngresosSupabaseForEntregas(fechaInicio = null) {
    // Si se proporciona fecha específica, no usar caché
    if (!fechaInicio && _entregasIngresosCache && Array.isArray(_entregasIngresosCache) && _entregasIngresosCache.length > 0) {
    ...
    // Filtrar por fecha
    const fechaFiltro = fechaInicio || (() => {
        const hace90Dias = new Date();
        hace90Dias.setDate(hace90Dias.getDate() - 90);
        return hace90Dias.toISOString().split('T')[0];
    })();

    while (true) {
        try {
            const url = `${CONFIG.SUPABASE_URL}/rest/v1/ingresos?select=*&fecha_ingreso=gte.${fechaFiltro}`;
    ...
    // Solo cachear si no hay fecha específica
    if (!fechaInicio) {
        _entregasIngresosCache = allRows;
    }
    
    console.log(`✅ Ingresos cargados: ${allRows.length} registros desde ${fechaFiltro}`);
    return allRows;
}

// Obtener datos de DATA2 desde la tabla ingresos de Supabase
async function obtenerDatosDeData2(fechaInicio = null) {
    try {
        const rows = await fetchIngresosSupabaseForEntregas(fechaInicio);
...
async function obtenerDatosRecFiltrados(fechaInicio = null) {
    try {
        const rows = await fetchIngresosSupabaseForEntregas(fechaInicio);
