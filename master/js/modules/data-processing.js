...
async function processCSVData(rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (groupedDataMap.has(groupKey)) {
            ...
            // Calcular sufijo: contar cuántas veces aparece esta OP en DATA2 col G
            const ocurrencias = data2CountMap.get(op) || 0;
            const opConSufijo = ocurrencias > 0 ? `${op}.${ocurrencias + 1}` : op;
            ...
            groupedDataMap.set(groupKey, {
                ...
                USUARIO: escanersMap.get(usuario) || usuario,
                OP: op,
                OP_SUFIJO: opConSufijo,
                ES_PARCIAL: ocurrencias > 0,
