    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (response.status === 429) {
                if (attempt === MAX_RETRIES) throw new Error(`HTTP 429: Rate limit alcanzado para ${range}`);
                console.warn(`⏳ Rate limit (429) en ${range}, reintentando en ${delay}ms... (intento ${attempt}/${MAX_RETRIES})`);
...
async function loadAllConfigData() {
    ...
    // Serializado para no saturar la API
    await loadUsuariosData();
    await loadProveedoresData();
    await loadAuditoresData();
    await loadGestoresData();
...
async function loadData2Data() {
    ...
    if (data.values && data.values.length > 0) {
        ...
        const loadTime = performance.now() - startTime;
        console.log(`✅ DATA2 cargada: ${data2Map.size} registros en ${loadTime.toFixed(0)}ms`);
    }
    return data2Map;
}
...
async function fetchDistributionSheet(range) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        const response = await fetch(url);
        if (response.status === 429) {
            if (attempt === MAX_RETRIES) throw new Error(`HTTP 429: Rate limit en distribución`);
            console.warn(`⏳ Rate limit distribución, reintentando en ${delay}ms...`);
            await new Promise(r => setTimeout(r, delay));
            delay *= 2;
            continue;
        }
...
/**
 * Guarda el estado completo de pedidos en la hoja PEDIDOS
 */
async function savePedidosData(pedidosObj) {
    const formData = new URLSearchParams();
    formData.append('action', 'savePedidos');
    formData.append('datos', JSON.stringify(pedidosObj));
    const response = await fetch(SISPROWEB_GAS_URL, { method: 'POST', body: formData });
    if (!response.ok) throw new Error(`HTTP Error: ${response.status}`);
    const result = await response.json();
    if (!result.success) throw new Error(result.message || 'Error guardando pedidos');
    return result;
}

async function loadPedidosData() {
    const formData = new URLSearchParams();
    formData.append('action', 'loadPedidos');
...
async function saveFinalizadosData(obj) {
    const formData = new URLSearchParams();
    formData.append('action', 'saveFinalizados');
    formData.append('datos', JSON.stringify(obj));
    const response = await fetch(SISPROWEB_GAS_URL, { method: 'POST', body: formData });
    if (!response.ok) throw new Error(`HTTP Error: ${response.status}`);
    const result = await response.json();
    if (!result.success) throw new Error(result.message || 'Error guardando finalizados');
    return result;
}

async function loadFinalizadosData() {
    const formData = new URLSearchParams();
    formData.append('action', 'loadFinalizados');
    formData.append('datos', '{}');
    const response = await fetch(SISPROWEB_GAS_URL, { method: 'POST', body: formData });
    if (!response.ok) throw new Error(`HTTP Error: ${response.status}`);
    const result = await response.json();
    if (!result.success) throw new Error(result.message || 'Error cargando finalizados');
    return result.data?.datos ?? '[]';
}
