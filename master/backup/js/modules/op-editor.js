        ...
        if (response.success) {
            try {
                ...
                if (typeof data2Map !== 'undefined' && jsonData.A) {
                    const op       = (jsonData.A       || '').toString().trim();
                    const fecha    = normalizeFecha(jsonData.FECHA || '');
                    const cantidad = (jsonData.CANTIDAD || '').toString().trim();
                    data2Map.set(`${op}|${fecha}|${cantidad}`, true);
                    // Actualizar conteo para sufijos
                    if (typeof data2CountMap !== 'undefined') {
...
function resetUIAfterSave(opGuardada) {
    ...
    if (opGuardada && selectOP) {
        const optionToRemove = Array.from(selectOP.options)
            .find(opt => opt.value && JSON.parse(opt.dataset.items || '[]')[0]?.OP?.toString() === opGuardada.toString());
        if (optionToRemove) selectOP.removeChild(optionToRemove);
    }
