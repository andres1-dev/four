...
Deno.serve(async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'upsert') {
      try {
        ...
        console.log(`📦 Recibidas ${entregas.length} entregas para upsert`)
        ...
        // Preparar entregas para inserción
        ...
        if (error) {
          ...
          console.log(`✅ ${results.success} entregas insertadas`)
        }
    ...
    if (action === 'delete') {
      try {
        ...
        let querySelect = supabaseClient.from('ENTREGAS').select('*')
        ...
        if (imagenesAEliminar.length > 0) {
          ...
          const { data: deletedFiles, error: storageError } = await supabaseClient
            .storage
        ...
        let queryDelete = supabaseClient.from('ENTREGAS').delete()
  ...
  try {
    ...
    if (action === 'stats') {
      ...
      const { data: facturas, error: facturasError } = await supabaseClient
        .from('SIESA')
        .select('"Nro documento"')
        .neq('Estado', 'Anuladas')
