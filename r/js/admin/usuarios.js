        ...
        ${paginatedData.map(item => {
            ...
            <div class="admin-card-lux" style="background:#fff; border:1px solid #f1f5f9; border-radius:16px; box-shadow:0 2px 8px rgba(0,0,0,0.04); overflow:hidden; display:flex; flex-direction:column;">
                ...
                <div style="padding:12px 16px; border-top:1px solid #f8fafc; background:#fafbfc; display:flex; gap:8px;">
                    ...
                    <button onclick="toggleUserStatus('${id}', '${rol}', ${isPlant})" title="${rol === 'DESHABILITADO' ? 'Habilitar usuario' : 'Deshabilitar usuario'}" style="padding:8px 12px; background:${rol === 'DESHABILITADO' ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#dc2626,#ef4444)'}; color:#fff; border:none; border-radius:10px; font-size:0.75rem; font-weight:700; cursor:pointer; display:flex; align-items:center; gap:6px;">
                        <i class="fas ${rol === 'DESHABILITADO' ? 'fa-user-check' : 'fa-user-slash'}"></i>
                    </button>
