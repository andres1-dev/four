...
async function saveToSheets() {
    ...
    const extraInfoFormateado = diferencia > 0 
        ? `<div style="text-align: center; margin-top: 16px; color: var(--warning); font-size: 14px;">Hay <strong>${diferencia} unidades faltantes</strong> en la orden.</div>` 
        : ``;

    const tableFooterHTML = `
        <tr style="border-top: 1px dashed var(--border);">
            <td style="text-align: center; padding: 12px; color: var(--text); font-weight: 700; font-size: 13px;">COSTO TOTAL</td>
            <td style="text-align: center; padding: 12px; color: var(--text); font-weight: 700; font-size: 14px;">${costoFormateado}</td>
        </tr>
    `;
    ...
    const confirmed = await showQuickConfirm(
        ...
        diferencia > 0
            ? `Detalle de unidades procesadas:`
