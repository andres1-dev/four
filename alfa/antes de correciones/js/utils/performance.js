...
function startTimer(label) {
    ...
    Logger.info('performance', `⏱️  ${label} iniciado...`);
    
    return function() {
        const end = performance.now();
        const duration = end - start;
        Logger.info('performance', `⏱️  ${label} completado en ${duration.toFixed(2)}ms`);
        return duration;
    };
}
...
Logger.success('performance', '✅ Performance utilities cargadas');