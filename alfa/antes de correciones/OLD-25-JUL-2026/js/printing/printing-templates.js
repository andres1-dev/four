...
function print_abrirPlantillaImpresion(datos, options = {}) {
    ...
    if (!ventana) { alert('Ventana bloqueada por el navegador.'); return; }
...
function print_imprimirLoteDocumentos(listaProcesada, titulo = null) {
    ...
    // Serializar los items para pasarlos al script interno de la ventana
    const itemsJSON = JSON.stringify(listaProcesada.map(item => ({
        datos: item.datos,
    ...
    <head>
        ...
        ${print_getEstilosOriginales()}
    </head>
    <body class="lote-body">
        <div class="pill-bar no-print">
            <button onclick="imprimirEnSecuencia()" class="btn-pill-primary">Imprimir</button>
    ...
    listaProcesada.forEach((item) => {
        ...
        <div class="lote-separator">
            <div class="print-unit">
                ${print_generarContenidoInterno(item.datos, item.options)}
            </div>
        </div>`;
    });

    // Script de secuencia embebido: al hacer click en Imprimir abre una pestaña por item
    htmlLote += `
        ${print_getScriptsOriginales()}
        <script>
        var _loteItems = ${itemsJSON};

        function imprimirEnSecuencia() {
            if (window.opener && window.opener._iniciarSecuenciaDesdeOpener) {
                window.opener._iniciarSecuenciaDesdeOpener(_loteItems, window);
            } else {
                alert('No se puede comunicar con la página principal. Cierra esta ventana y vuelve a intentarlo.');
            }
        }
        <\/script>
    </body></html>`;

    // Registrar en la página principal la función que abrirá las pestañas hijas
    // para que todos los window.open salgan del origen con permisos ya concedidos
    window._iniciarSecuenciaDesdeOpener = function(items, ventanaLote) {
        let index = 0;

        function abrirSiguiente() {
            if (index >= items.length) {
                delete window._imprimirSiguiente;
                delete window._iniciarSecuenciaDesdeOpener;
                if (ventanaLote && !ventanaLote.closed) ventanaLote.close();
                
                // Limpiar el input de impresión al terminar la secuencia
                limpiarInputImpresion();
                return;
            }
            const item = items[index++];
            window._imprimirSiguiente = abrirSiguiente;

            const html = print_generarDocumentoCompleto(item.datos, item.options, true);
            const v = window.open('', '_blank');
            if (!v) { alert('El navegador bloqueó una ventana. Permite popups para este sitio.'); return; }
...
/**
 * Imprime cada cliente en su propia pestaña en secuencia.
 * onDone=true inyecta autoprint + callback al padre para abrir el siguiente.
 */
function print_imprimirClientesEnSecuencia(datos, clientes) {
    ...
    function abrirSiguiente() {
        ...
        const ventana = window.open('', '_blank');
        if (!ventana) { alert('El navegador bloqueó una ventana. Permite popups para este sitio.'); return; }
        ventana.document.write(html);
        ventana.document.close();
    }
...
/**
 * Genera el HTML completo de un documento.
 * onDone=true → inyecta script que espera imágenes, imprime y llama al siguiente.
 */
function print_generarDocumentoCompleto(datos, options = {}, onDone = false) {
    ...
    const scriptAutoprint = onDone ? `<script>
        window.addEventListener('load', function() {
            var imgs = document.querySelectorAll('img');
            var total = imgs.length;
            function doprint() {
                window.print();
                if (window.opener && window.opener._imprimirSiguiente) window.opener._imprimirSiguiente();
                window.close();
            }
