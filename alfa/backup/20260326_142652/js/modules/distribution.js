...
function evaluateAndAdjustSecondaryCompanies(recData) {
    ...
    // Calcular cuántas filas tienen cantidad suficiente para distribución
    let rowsWithSufficientQuantity = 0;
    let totalRows = recData.HR.length;
    
    recData.HR.forEach(row => {
        const cantidad = parseInt(row[3]) || 0;
        if (cantidad >= minQuantityForDistribution) {
            rowsWithSufficientQuantity++;
        }
    });
    
    // Si menos del 50% de las filas tienen cantidad suficiente, resetear secundarias a 0
    const percentageWithSufficientQuantity = (rowsWithSufficientQuantity / totalRows) * 100;
    
    if (percentageWithSufficientQuantity < 50) {
        ...
        if (typeof showMessage === 'function') {
            showMessage(`Cantidades insuficientes detectadas. Distribución ajustada a 100% empresa principal.`, 'info', 3000);
        }
    } else {
        // Cantidades suficientes: restaurar valores predeterminados
        empresasData.forEach(([id, config]) => {
            if (config.tipoEmpresa === "Secundaria") {
                const input = document.getElementById(`empresa-${id}`);
                if (input) {
...
function updateAllDistributionValues() {
    ...
    // Obtener la cantidad mínima configurada
    const minQuantityForDistribution = parseInt(localStorage.getItem('minQuantityForDistribution')) || 120;
    ...
    currentRecData.HR.forEach((row, rowIndex) => {
        ...
        // REGLA: Si la cantidad disponible es menor a la cantidad mínima, todo va a la empresa principal
        if (availableForCompanies < minQuantityForDistribution) {
            // Asignar todo a la empresa principal
            empresasData.forEach(([id, config]) => {
                if (config.tipoEmpresa === "Secundaria") {
                    const secundariaCell = tr.querySelector(`.secundaria-cell[data-empresa="${id}"]`);
                    if (secundariaCell) secundariaCell.textContent = '0';
                }
            });
            
            const principalCell = tr.querySelector('.principal-cell');
            if (principalCell) principalCell.textContent = availableForCompanies;
        } else {
