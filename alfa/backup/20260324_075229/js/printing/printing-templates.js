...
function print_imprimirLoteDocumentos(listaProcesada, titulo = null, options = {}) {
    if (!listaProcesada || listaProcesada.length === 0) return;

    // Usar la constante global si existe, o las opciones pasadas
    const duplex = options.duplex !== undefined ? options.duplex : (typeof IMPRESION_DUPLEX !== 'undefined' ? IMPRESION_DUPLEX : true);
    const ventana = window.open('', '_blank');
    ...
    <head>
        ...
        ${print_getEstilosOriginales(true, duplex)}
    </head>
...
/**
 * ESTILOS (COPIA EXACTA DEL BACKUP ORIGINAL + SOPORTE DUPLEX)
 */
function print_getEstilosOriginales(esLote = false, duplex = true) {
    return `
    <style>
        @page {
            size: letter;
        ...
        @media print {
            ...
            /* LOGICA DUPLEX CRITICA - SOPORTE PARA LADO Y LADO EFECTIVO */
            .lote-separator {
                display: block !important;
                width: 100% !important;
                ${duplex ? 'break-after: right !important; break-after: recto !important; break-before: right !important; break-before: recto !important; page-break-after: right !important; page-break-before: right !important;' : 'break-before: page !important; page-break-before: always !important;'}
