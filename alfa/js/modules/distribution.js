...
function agregarRecAInputImpresion(recNumber) {
    const printInput = document.getElementById('printRecInput');
    if (!printInput) return;
    
    const currentValue = printInput.value.trim();
    
    // Si el input está vacío, agregar el REC directamente
    if (!currentValue) {
        printInput.value = recNumber;
        return;
    }
    
    // Si ya contiene el REC, no agregarlo de nuevo
    const recsArray = currentValue.split(',').map(r => r.trim());
    if (recsArray.includes(recNumber)) {
        return;
    }
    
    // Agregar el REC con coma
    printInput.value = currentValue + ',' + recNumber;
}
