...
- **🚨 `validarEstadoParcial(lote, traslado, totalGeneral, cantidad)`:** The advanced "PARTIAL" validation core.
  ...
  - Determinations:
    - `{rechazar: true, tipo: 'REPETIDO'}`: Blocked because the `traslado` (Transfer ID) is already processed in the historic JSON — checked against `TRASLADO` (header), `OTROS_TRASLADOS` (secondary DI transfers), and `ANEXOS[].TRASLADO`.
...
- **🚨 `generateJSONForOP()`:** Re-assembles all fields and input checks validating strictly.
  ...
  - Master Structural Architecture built on the fly:
    ...
    - **`OTROS_TRASLADOS` (Array of strings):** Collects ALL unique transfer numbers from every item in the OP (HR + ANEXOS), excluding the main `TRASLADO` header value. Solves the case where additional DI entries arrive from secondary transfers (e.g., pending transfers from a previous day). Used by `validarEstadoParcial` to detect these as REPETIDO on future CSV loads.
