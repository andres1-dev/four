function showQuickConfirm(title, message, confirmText = 'Confirmar', cancelText = 'Cancelar', type = 'warning', tableData = null, tableFooterHTML = '', extraInfoFormateado = '') {
    return new Promise((resolve) => {
        const iconConfig = {
            'warning': { icon: 'codicon-warning', color: 'var(--warning)', bg: 'var(--warning-dim)' },
            'error': { icon: 'codicon-error', color: 'var(--error)', bg: 'var(--error-dim)' },
            'info': { icon: 'codicon-info', color: 'var(--info)', bg: 'var(--info-dim)' },
            'success': { icon: 'codicon-check', color: 'var(--success)', bg: 'var(--success-dim)' }
        };
        ...
        const modal = createModal('', `
            <div style="padding: 24px; text-align: center;">
                <div class="alert-icon" style="margin: 0 auto 20px; background: ${config.bg}; border-radius: 50%; width: 64px; height: 64px; display: flex; align-items: center; justify-content: center;">
                ...
                <div style="display: flex; gap: 12px; justify-content: center; margin-top: 24px;">
                    <button class="btn-secondary btn-cancel" style="min-width: 100px; padding: 10px 20px;">${cancelText}</button>
                    <button class="btn-primary btn-confirm" style="min-width: 120px; padding: 10px 20px; background-color: ${config.color}; border-color: ${config.color};">
        ...
        modal.querySelector('.btn-cancel').onclick = () => closeHandler(false);
        modal.querySelector('.btn-confirm').onclick = () => closeHandler(true);
    });
}
...
function showQuickChoice(title, message, options) {
    return new Promise((resolve) => {
        const modal = createModal(title, `
            <div style="padding: 24px; max-width: 500px;">
                ...
                <div style="display: flex; justify-content: flex-end;">
                    <button class="btn-secondary btn-close">Cancelar</button>
                </div>
            </div>
        `, false);
        ...
        modal.querySelectorAll('.choice-option').forEach(button => {
            button.onclick = () => closeHandler(button.dataset.choice);
        });

        const closeBtn = modal.querySelector('.btn-close');
        if (closeBtn) closeBtn.onclick = () => closeHandler(null);
    });
}