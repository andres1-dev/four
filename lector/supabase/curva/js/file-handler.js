...
const FileHandlerCurva = {
  ...
  readFile(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log('✅ Registros extraídos:', this.parsedData.length);
        console.log('📦 Ejemplo de registro:', this.parsedData[0]);
  ...
  extractTallasHeaders(data) {
    ...
    console.log('📋 Fila de headers (índice 1):', headerRow);
    console.log('🔢 Buscando tallas desde columna índice:', CurvaConfig.COLUMNS.TALLAS_START);
    ...
    console.log(`👕 Tallas encontradas (${this.tallasHeaders.length}):`, this.tallasHeaders.map(t => t.name).join(', '));
  },

  extractData(data) {
    const result = [];
    
    console.log(`📊 Procesando datos desde fila ${CurvaConfig.DATA_START_ROW + 1} (índice ${CurvaConfig.DATA_START_ROW})`);
    ...
    console.log(`📦 Total de registros expandidos por talla: ${allRecords.length}`);
    ...
    console.log(`✅ Total de OPs únicas: ${finalRecords.length}`);
    console.log('📦 Ejemplo de OP agrupada:', finalRecords[0]);
