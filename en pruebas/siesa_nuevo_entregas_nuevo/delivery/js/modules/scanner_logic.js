...
class QRScanner {
  constructor() {
    ...
    log.info("QR Scanner Module Initialized - Robust Version");
  }
  ...
  onScanSuccess(decodedText) {
    ...
    if (this.isProcessingCode) {
      log.info('⏭️ Lectura duplicada ignorada (ya procesando)');
      return;
    }
    
    log.success('✅ QR escaneado:', decodedText);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    try {
      ...
      log.success('QR Scanner initialized successfully');
    } catch (error) {
      log.error('Failed to initialize QR Scanner:', error);
    }
  }, 500);
});
