...
// ⚠️ IMPORTANTE: La API Key se obtiene dinámicamente desde el servidor al hacer login
// y se almacena en localStorage. No usar CONFIG.GOOGLE_SHEETS_API_KEY directamente.
const getApiKey = () => {
    const apiKey = localStorage.getItem('apiKey');
    if (!apiKey) {
        console.error('❌ API Key no disponible. Por favor inicia sesión nuevamente.');
        return null;
    }
    return apiKey;
};
...
async function obtenerDatosFacturados() {
    try {
        log.info("Iniciando obtención de datos facturados...");
        ...
        log.info("Estadísticas iniciales:");
        log.info("- DATA2:", datosData2.length, "registros");
        log.info("- SIESA:", datosSiesa.length, "facturas");
        log.info("- REC:", datosRec.length, "registros FULL");
        log.info("- SOPORTES:", Object.keys(datosSoportes).length, "confirmaciones");
        ...
        log.info("Resultado:", resultado.metadata.estadisticas);

        return resultado;

    } catch (error) {
        log.error("Error en obtenerDatosFacturados:", error);
        return {
            success: false,
            error: error.message,
...
function combinarDatosFacturados(datosData2, datosSiesa, datosSoportes, datosRec) {
    ...
    log.info("🔄 Combinando datos...");
    log.info(`📦 DATA2: ${datosData2.length} registros`);
    log.info(`📦 REC: ${datosRec.length} registros`);
    log.info(`✅ Soportes: ${Object.keys(datosSoportes).length} facturas entregadas`);
    ...
    log.info(`   - Documentos: ${datosCombinados.length} (DATA2: ${desdeDATA2}, REC: ${desdeREC}, HUÉRFANAS: ${desdeHuerfanas})`);
    log.info(`   - Facturas: ${totalFacturas}`);
    log.info(`   - Entregadas: ${entregadas}`);
    log.info(`   - Pendientes: ${totalFacturas - entregadas}`);
    ...
    try {
        ...
        return allValues.reduce((filtrados, cellValue, index) => {
            ...
            try {
                ...
                // Silenciar warning de JSON inválido en producción
                log.warn(`Fila ${index + 1} DATA2 no es JSON válido`);
            }
            return filtrados;
        }, []);
...
async function obtenerDatosSoportes() {
    try {
        log.info('📦 Cargando entregas desde Supabase...');
        ...
        if (!result.entregas || result.entregas.length === 0) {
            log.info('📊 No hay entregas en Supabase');
            return {};
        }

        log.success(`✅ Recibidas ${result.entregas.length} entregas desde Supabase`);
        ...
        log.success(`✅ Procesadas ${Object.keys(mapaSoportes).length} facturas únicas de ${result.entregas.length} registros totales`);
        return mapaSoportes;

    } catch (error) {
        log.error("❌ Error en obtenerDatosSoportes:", error);
        
        // Si es error de autenticación, redirigir al login
        if (error.message.includes('sesión') || error.message.includes('Sesión expirada')) {
...
if (typeof window !== 'undefined') {
    ...
    log.info("principal.js cargado - funciones disponibles:");
    log.info("- obtenerDatosFacturados()");
    log.info("- ejecutarYMostrarResultados()");
    log.info("- exportarDatosComoJSON(datos)");
}
