...
// ⚠️ DEPRECADO: Este archivo usa autenticación con Google Sheets
// Se recomienda usar auth_supabase.js en su lugar

// Función para obtener usuarios desde Google Sheets
async function obtenerUsuariosDeSheets() {
    // Obtener API Key desde localStorage (obtenida al hacer login con Supabase)
    const API_KEY = localStorage.getItem('apiKey');
    if (!API_KEY) {
        log.error('❌ API Key no disponible');
        throw new Error('API Key no disponible. Por favor inicia sesión.');
    }
    ...
    try {
        ...
        if (values.length <= 1) {
            log.warn("No hay usuarios en la hoja USERS");
            return [];
        }
        ...
        log.success(`✅ Usuarios cargados desde Sheets: ${users.length}`);
        return users;
        
    } catch (error) {
        log.error("❌ Error obteniendo usuarios de Sheets:", error);
        throw error;
    }
}
    ...
    try {
        ...
        if (userFound) {
            return {
                ...
                apiKey: localStorage.getItem('apiKey') || '' // Usar la API Key del localStorage
            };
        } else {
            return {
                success: false,
        ...
        log.error("Error en verificación:", error);
        return {
            success: false,
...
async function loginBiometricoSheets(id) {
    ...
    try {
        ...
        log.error("Error en login biométrico:", error);
        return {
            success: false,
            message: 'Error de conexión con el servidor'
        };
...
async function recuperarContrasenaSheets(id) {
    ...
    try {
        ...
        log.error("Error en recuperación:", error);
        return {
            success: false,
            message: 'Error de conexión con el servidor'
        };
...
if (typeof window !== 'undefined') {
    ...
    log.info("auth_sheets.js cargado - Autenticación con Google Sheets activa");
}
