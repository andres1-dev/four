...
function initUIListeners() {
    ...
    if (typeof window.APP_MODE === 'undefined') {
        ...
        log.info('🔧 Modo inicial cargado:', window.APP_MODE);
    }
    ...
    try {
        ...
        if (rDateInput && typeof flatpickr !== 'undefined') {
            log.info("Initializing Flatpickr for Report");
        ...
        if (reportBtn) {
            log.info("Report button found, attaching listener");
    ...
    function initPDAModes() {
        ...
        function setMode(mode) {
            ...
            log.info('💾 Guardando modo:', mode);
            if (typeof saveAppMode === 'function') {
                saveAppMode(mode);
                log.success('✅ Modo guardado en localStorage');
            } else {
                log.error('❌ saveAppMode no está definida');
            }

            // ⭐ GESTIÓN DE FOCO PERSISTENTE SEGÚN MODO
            if (mode === 'PDA') {
                // ✅ MODO PDA: SIEMPRE activar foco persistente
                log.info('🔒 Modo PDA - ACTIVANDO foco persistente');
                ...
                log.info('🔓 Modo no-PDA - DESACTIVANDO foco persistente');
        ...
        log.info('🎯 Aplicando modo guardado:', savedMode);
        
        if (savedMode === 'MANUAL') {
            const hasClient = (typeof USER_SETTINGS !== 'undefined' && USER_SETTINGS.filterEnabled && USER_SETTINGS.selectedClient);
            if (hasClient) {
                log.success('✅ Activando modo MANUAL con cliente:', USER_SETTINGS.selectedClient);
                setMode('MANUAL');
            } else {
                log.warn('⚠️ Modo MANUAL requiere cliente seleccionado, cambiando a CAMERA');
                setMode('CAMERA');
            }
        } else {
            log.success('✅ Activando modo:', savedMode);
            setMode(savedMode);
        }
    }
    ...
    if (screen.orientation && screen.orientation.lock) {
        screen.orientation.lock('portrait').catch(function (error) {
            log.warn('Rotación no bloqueada:', error);
        });
    }
