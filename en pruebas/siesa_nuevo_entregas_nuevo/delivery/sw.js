...
// ============================================
// NOTIFICACIONES PUSH - DESHABILITADO
// ============================================
// Las notificaciones push y el polling han sido deshabilitados
// El código se mantiene comentado para referencia futura
...
self.addEventListener('install', (event) => {
  // Silenciado: console.log('[SW] 🔧 Instalando Service Worker...');
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        // Silenciado: console.log('[SW] Cacheando assets locales...');
        return cache.addAll(ASSETS_TO_CACHE)
          .then(() => {
            // Silenciado: console.log('[SW] Assets locales cacheados');
            return Promise.allSettled(
              EXTERNAL_ASSETS.map(url =>
                cache.add(url)
                  .then(() => {}) // Silenciado: console.log(`[SW] Cacheado: ${url}`)
                  .catch(err => {
                    // Mantener solo errores críticos
                    console.error(`[SW] Error cacheando ${url}:`, err.message);
                    return null;
                  })
              )
            );
      ...
      .then(() => {
        // Silenciado: console.log('[SW] Instalación completada');
...
self.addEventListener('activate', (event) => {
  // Silenciado: console.log('[SW] Activando Service Worker...');
  event.waitUntil(
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cache => {
          if (cache !== CACHE_NAME) {
            // Silenciado: console.log('[SW] Eliminando cache antiguo:', cache);
            return caches.delete(cache);
          }
        })
      );
    }).then(() => {
      // Silenciado: console.log('[SW] Service Worker activado');
...
self.addEventListener('message', (event) => {
  ...
  /* DESHABILITADO - Polling de notificaciones
...
/* DESHABILITADO - Sistema de polling
let pollingActive = false;
*/

/* ============================================
   POLLING DESHABILITADO - Código comentado para referencia
   ============================================
...
/* ============================================
   PUSH NOTIFICATIONS DESHABILITADO
   ============================================
...
/* DESHABILITADO - Limpieza de notificaciones
setInterval(() => {
  if (!processingNotification) {
...
// Silenciado: console.log('[SW] Service Worker cargado - Versión:', CACHE_NAME);

/* DESHABILITADO - Inicio automático de polling
// INICIO AUTOMÁTICO
startBackgroundPolling();
*/
