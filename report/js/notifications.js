...
const NOTIF_POLL_INTERVAL = 5_000; // 5 segundos — detección rápida de cambios de estado
let _notifPollTimer = null;
let _lastKnownStates = {}; // { ID_NOVEDAD: ESTADO }
let _initialLoadDone = false; // evita notificar en la primera carga si ya hay snapshot
let _notifications = []; // { id, nov, estadoAnterior, estadoActual, ts, read }
let _cachedNovedades = []; // última carga para el panel de seguimiento
...
function initNotifications(preloadedNovedades) {
    ...
    _notifPollTimer = setInterval(_pollNovedades, NOTIF_POLL_INTERVAL);
}
    ...
    if (isGuest) {
        ...
        panel.innerHTML = `
            ...
            ">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                    <button onclick="_markAllOperatorRead()" style="
                        background:rgba(255,255,255,0.2); border:none; color:white;
                        font-size:0.7rem; font-weight:700;
                        padding:4px 10px; border-radius:20px; cursor:pointer;
...
function _addNotifications(items) {
    items.forEach(item => {
        _notifications.unshift({
            id: `${item.nov.ID_NOVEDAD}_${Date.now()}`,
            nov: item.nov,
    ...
    _showToastNotification(items[0]);
}
